    ...
    WhoRecognitionButton(recognition)
{
    ...
    w = std::max(w, static_cast<int32_t>(lv_obj_get_width(m_btn_enroll)));
    w = std::max(w, static_cast<int32_t>(lv_obj_get_width(m_btn_delete)));
...
{
    bsp_display_lock(0);
    lv_obj_del(m_btn_recognize);
    lv_obj_del(m_btn_enroll);
    lv_obj_del(m_btn_delete);
    bsp_display_unlock();
}
